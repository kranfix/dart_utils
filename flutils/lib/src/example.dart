...
abstract base mixin class LinkedListEntry<E extends LinkedListEntry<E>> {
  WeakReference<LinkedList<E>>? __list;
  E? _next;
  WeakReference<E>? __previous;

  E? get _previous => __previous?.target;
  set _previous(E? entry) {
    if (entry != null) {
      __previous = WeakReference(entry);
    } else {
      __previous = null;
    }
  }
  ...
  LinkedList<E>? get list => __list?.target;
  LinkedList<E>? get _list => __list?.target;
  set _list(LinkedList<E>? list) {
    if (list != null) {
      __list = WeakReference(list);
    } else {
      __list = null;
    }
  }
